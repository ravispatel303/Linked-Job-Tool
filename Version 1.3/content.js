...
for (e in array){
	...
	if ( !( array[e].innerHTML.includes('</') || (new RegExp(ignore.join("|")).test(cText.toLowerCase())) ) ){
		...
		// if (document.querySelectorAll('#ember15').length) {
		// 	var child = document.querySelectorAll('#ember15')[0];	
		// 	console.log('ember15')
		// } 
		...
		// This is the improved code to display text(highlighted) on web page. 
		// The below code(one line) will work for both LinkedIn Premium and Normal users
		// This has solved the bug.
		// BUG: Sometimes id(ember15 or ember16) gets changed on webpage, results into error(display error).
		var child = document.getElementsByClassName('jobs-description--is-truncated')[0]
...
for (ele of job2) {
	...
	// note we've already covered tags like strong, p, li in above method this method is to consider remaining #text tag 
